...
def UltrasoundIndoor(data_root=None, split=None, download=False, **kwargs):
    """
    Convenience function for loading UltrasoundIndoor dataset.

    Args:
        data_root: Root directory for dataset storage
        split: Dataset split ('train', 'test', 'all', or None for tuple)
        download: Whether to download if not found
        **kwargs: Additional arguments passed to UltrasoundIndoorDataset
    ...
    Examples:
        ...
        >>> train, test = UltrasoundIndoor(download=True)

        >>> # Load entire dataset (train + test merged)
        >>> dataset = UltrasoundIndoor(split='all', download=True)

        >>> # Load only training set
        >>> train = UltrasoundIndoor(split='train', download=True)
    """
    if split is None:
        # Return both train and test as tuple
        train_dataset = UltrasoundIndoorDataset(
            data_root=data_root,
            split='train',
            download=download,
            **kwargs
        )
        test_dataset = UltrasoundIndoorDataset(
            data_root=data_root,
            split='test',
            download=download,
            **kwargs
        ...
        # Return single split
        return UltrasoundIndoorDataset(
            data_root=data_root,
            split=split,
            download=download,
            **kwargs
