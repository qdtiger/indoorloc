...
def VLCIndoor(data_root=None, split=None, download=False, **kwargs):
    """
    Convenience function for loading VLCIndoor dataset.

    Args:
        data_root: Root directory for dataset storage
        split: Dataset split ('train', 'test', 'all', or None for tuple)
        download: Whether to download if not found
        **kwargs: Additional arguments passed to VLCIndoorDataset
    ...
    Examples:
        ...
        >>> train, test = VLCIndoor(download=True)

        >>> # Load entire dataset (train + test merged)
        >>> dataset = VLCIndoor(split='all', download=True)

        >>> # Load only training set
        >>> train = VLCIndoor(split='train', download=True)
    """
    if split is None:
        # Return both train and test as tuple
        train_dataset = VLCIndoorDataset(
            data_root=data_root,
            split='train',
            download=download,
            **kwargs
        )
        test_dataset = VLCIndoorDataset(
            data_root=data_root,
            split='test',
            download=download,
            **kwargs
