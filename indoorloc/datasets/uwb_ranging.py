...
def UWBRanging(data_root=None, split=None, download=False, **kwargs):
    """
    Convenience function for loading UWBRanging dataset.

    Args:
        data_root: Root directory for dataset storage
        split: Dataset split ('train', 'test', 'all', or None for tuple)
        download: Whether to download if not found
        **kwargs: Additional arguments passed to UWBRangingDataset
    ...
    Examples:
        ...
        >>> train, test = UWBRanging(download=True)

        >>> # Load entire dataset (train + test merged)
        >>> dataset = UWBRanging(split='all', download=True)

        >>> # Load only training set
        >>> train = UWBRanging(split='train', download=True)
    """
    if split is None:
        # Return both train and test as tuple
        train_dataset = UWBRangingDataset(
            data_root=data_root,
            split='train',
            download=download,
            **kwargs
        )
        test_dataset = UWBRangingDataset(
            data_root=data_root,
            split='test',
            download=download,
            **kwargs
